...
derive_returns_hazards_from_Rx <- function(age,
                                           ...
                                           smooth_w = 0,          # penalize (log hu - log hu_prev)^2 + (log uh - log uh_prev)^2
  ...
  # helpers
  clamp <- function(z) pmin(bounds[2], pmax(bounds[1], z))
  ...
  hu0 <- clamp(make_start(hu_start, default = 1e-3))
  uh0 <- clamp(make_start(uh_start, default = 1e-3))
  ...
  for (i in seq_len(n - 1)) {
    ...
    # previous solution for smoothness penalty
    if (i == 1) {
      log_prev <- c(log(hu0[i]), log(uh0[i]))
    } else {
      log_prev <- c(log(hu[i - 1]), log(uh[i - 1]))
    }
    
    # objective in log-space
    obj <- function(logx) {
      # logx = c(log_hu, log_uh)
      hu_i <- clamp(exp(logx[1]))
      uh_i <- clamp(exp(logx[2]))
      ...
      if (smooth_w > 0) {
        loss <- loss + smooth_w * sum((logx - log_prev)^2)
      }
      ...
      if (bound_w > 0) {
        ...
        eps <- 1e-12
        hu_scaled <- (hu_i - bounds[1]) / (bounds[2] - bounds[1] + eps)
        uh_scaled <- (uh_i - bounds[1]) / (bounds[2] - bounds[1] + eps)
        # blow up near 0 or 1
    ...
    hu[i] <- clamp(exp(res$par[1]))
    uh[i] <- clamp(exp(res$par[2]))
